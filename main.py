...
class Extensions:
    ...
    def get_gpt_generation(keyword: str, lang: str = "",mode: str = "body") -> Optional[str]:
        ...
        if mode == "body":
            prompts = f"""以<{keyword}>为标题，，并结合行业特点写一篇{lang}爆款科普性文章，只需以Markdown格式输出文章内容，不再需要输出标题和其他解释。
...
- 替换或减少机械式连接词（如：首先、其次、然后、再次、最后、总之、总而言之、然而、因此、另外、此外），改用更基础、常用、甚至口语化的表达，以增加自然度和可读性。
